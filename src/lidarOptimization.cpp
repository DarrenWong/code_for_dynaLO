...
bool SurfNormAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // residuals[0] = parameters[1][0]* residuals[0];
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
...
DynamicSurfNormAnalyticCostFunction::DynamicSurfNormAnalyticCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d plane_unit_norm_, double negative_OA_dot_norm_) 
                                                        : curr_point(curr_point_), plane_unit_norm(plane_unit_norm_), negative_OA_dot_norm(negative_OA_dot_norm_){

}

bool DynamicSurfNormAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    Eigen::Map<const Eigen::Quaterniond> q_w_curr(parameters[0]);
    Eigen::Map<const Eigen::Vector3d> t_w_curr(parameters[0] + 4);
    Eigen::Vector3d point_w = q_w_curr * curr_point + t_w_curr;
    double prev_residual = plane_unit_norm.dot(point_w) + negative_OA_dot_norm;
    double weight = parameters[1][0];
    residuals[0] = weight * prev_residual;
    // residuals[0] = parameters[1][0]* residuals[0];
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
            ...
            dp_by_se3.block<3, 3>(0, 3) = Eigen::Matrix3d::Identity() * weight;
